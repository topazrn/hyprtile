...
### The Gnome 45 version of HyprTile has undergone a rewrite! If you experience any regressions please check PR #341 for a summary of breaking changes. If you experience an undocumented regression please open a new issue.

- Gnome-shell extension that improves window tiling capabilities of stock gnome-shell.
- HyprTile is used to move/resize windows on a configurable grid scheme.
...
**Acknowledgement**: The original idea and the first implementation of HyprTile was developed by [topazrn](https://github.com/topazrn) with the help from multiple contributors. HyprTile is now a community supported project and licensed under the [GPL v2+](https://www.gnu.org/licenses/gpl-2.0.html).
...
The preferred installation is through [Gnome Extensions](https://extensions.gnome.org/extension/28/hyprtile/).

### Install Latest Build
Alternatively, the most recent stable version may also be downloaded as a distributable archive (`hyprtile.dist.tgz`) through the [GitHub releases page](https://github.com/HyprTile/HyprTile/releases).

```shell
# Replace `VERSION` with the most recent release.
wget https://github.com/HyprTile/HyprTile/releases/download/VERSION/hyprtile.dist.tgz
...
HyprTile can be used either through its graphical user interface ("overlay") or exclusively with the keyboard via shortcuts.

### Overlay

1. Make sure the window you want to resize has focus
2. Click on the HyprTile icon on the tool bar, or press `<Super>+<Enter>` (default)
3. The HyprTile dialog pop-up will show up at your cursors position
    - The ![Toggle auto-close icon](https://raw.githubusercontent.com/HyprTile/HyprTile/master/images/icons/dark/32/auto-close.png) button toggles the auto-closing of the HyprTile window after applying the changes.
...
* If grid size is specified in the first place, it can be omitted in the subsequent places. HyprTile will use the grid size specified in the first place. E.g. `3x3 1:1 1:1, 2:2 2:2` (when `2:2 2:2` is triggered, HyprTile will use 3x3 grid size).
...
1. Activate HyprTile by pressing `Super`+`KP_Enter` or clicking on HyprTile icon
...
)](https://github.com/HyprTile/HyprTile/assets/3457747/40489511-1c5d-44da-b59f-9445cf50b86c "Main Variants")
...
HyprTile is intended to **supplement** existing Gnome-shell keyboard shortcuts.

Here are some useful Gnome built-ins to keep in mind when configuring HyprTile:
...
`(Monitor) Inset` | A user-configurable screen margin respected by all HyprTile features (window placement, autogrow, autotiling, …). It causes windows to keep a fixed distance (in pixel) to the monitor edge(s).
